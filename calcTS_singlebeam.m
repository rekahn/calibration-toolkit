...
% FOR SINGLE BEAM TRANSDUCER

function cal = calcTS_singlebeam(cal, compressed, filter_freq, plot_flag,thresh_flag)  
...
ytx = EK80calcSentSignal(cal, 1, 1);
...
% remove top 1% and bottom 90% of pings
    ...
    percent90 = ceil(0.90 * length(pvec));
    [~,sortidx] = sort(pvec);
    top1 = sortidx(end-percent1:end);
    bot90 = sortidx(1:percent90);
    pingsFFT(:,[top1 bot90]) = [];
end
...
zet = cal.Z;
Per = 1*(abs(pingsFFT)/(2*sqrt(2))).^2 *((zer+zet)/zer)^2.*1/zet;
